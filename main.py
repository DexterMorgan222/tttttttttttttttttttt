...
# Переменная со всем твоим кодом интерфейса (без шапки yortAI PRO Mobile)
HTML_CONTENT = """
<!DOCTYPE html>
<html lang="ru">
<head>
...
# Главная страница возвращает вшитый HTML
@app.get("/", response_class=HTMLResponse)
async def read_index():
    return HTML_CONTENT

# Эндпоинт для обработки сообщений и картинок через Gemini API
@app.post("/api/chat")
async def chat_endpoint(request: ChatRequest):
    # Вставь сюда свои настоящие рабочие ключи
    api_keys = [
        "ТВОЙ_ПЕРВЫЙ_КЛЮЧ_GEMINI",
        "ТВОЙ_ВТОРОЙ_КЛЮЧ_GEMINI"
    ]
    
    # Случайный выбор ключа для обхода лимитов
    selected_key = random.choice(api_keys)
    
    # URL для вызова модели Gemini 2.5 Flash
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={selected_key}"
    
    # Формируем структуру запроса к Google AI API
    parts = []
    ...
    try:
        ...
        # Вытаскиваем текстовый ответ из структуры ответа Gemini
        ai_text = response_data['candidates'][0]['content']['parts'][0]['text']
        return {"content": ai_text}
        
    except Exception as e:
        print(f"Ошибка при запросе к Gemini API: {e}")
        # Если что-то пошло не так, возвращаем читаемую ошибку пользователю в чат
        return {"content": "Произошла ошибка при обработке запроса нейросетью. Попробуйте еще раз."}
